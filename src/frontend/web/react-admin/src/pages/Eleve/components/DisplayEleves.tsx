...
const DisplayEleves: React.FC<DisplayElevesProps> = (props) => {

    const state = useSelector((state: ReduxProps) => state);
    const [eleves, setEleves] = useState<Eleve[]>(props.eleves);
    const [eleve, setEleve] = useState<Eleve>(props.eleves[0]);
    ...
    const onGridReady = useCallback(() => {
        ...
        elevesApi.elevesIndex('Bearer ' + token)
        .then((response) => {  
    ...
    const handleNewItem = () => {
        ...
        setModalTitle("Créer un nouvelle eleve")
    }
    ...
    const handleExportExcel = () => {
        if (gridRef.current && gridRef.current.api) {      
            ...
            if (rowData && rowData.length > 0) {
                ...
                XLSX.writeFile(workbook, 'eleves_data.xlsx');
            } else {
    ...
    const handleUpdateItem = () => {
        if (gridRef.current && gridRef.current.api) {      
            ...
            if (rowData && rowData.length === 1) {
                ...
                setModalTitle("Modifier une eleve")
    ...
    const proccessDeleteItem = () => {
        ...
        elevesApi.deleteEleve( 'Bearer ' + token, eleve.id)
        .then((response) => {  
        if(response && response.data){                    
            if(response.data.success === true){ 
                ...
                props.setDangerNotifDescription('Cette eleve a ete supprimee avec success')
  ...
  return (
      <div>
        ...
        {showCreateOrUpdateModal && <CreateOrUpdateEleveModal 
                                        mode={modalMode} 
                                        title={modalTitle} 
        ...
        {showDeleteModal && <DeleteItemModal 
                                itemName={eleve.firstName + '' + eleve.lastName} 
...
function mapStateToProps(state: ReduxProps): ReduxProps {
  ...
  export default connect(mapStateToProps)(DisplayEleves)
