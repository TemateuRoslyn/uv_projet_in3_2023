...
import { CLASSE_LEVEL, SEXE, SOLVABLE, STATUS, getClasseByName } from "../../../constants/ITEMS";
...
const CreateOrUpdateEleveModal: React.FC<ModalProps> = (props) => {
    ...
    if(keyword.length > 0){
        ...
        classesApi.classesRecords(keyword, 'Bearer ' + token)
        .then((response) => {  
    ...
    const handleCreate = (_event: React.FormEvent) => {
        ...
        const token: string = localStorage.getItem(TOKEN_LOCAL_STORAGE_KEY)!;
        const elevesApi = new ElevesApi({...state.environment, accessToken: token});

        setIsLoading(true)        

        elevesApi.createEleve(
            formValues.email, 
        ...
        if(response && response.data){                    
            if(response.data.success === true){ 
                ...
                props.setSuccessNotifDescription('Une nouvelle eleve viens d\'etre rajoutee au catalogue avec success ! ')
    ...
    const handleUpdate = (_event: any) => {
        ...
        if(response && response.data){                    
            if(response.data.success === true){ 
                ...
                // notification
                props.setSuccessNotifMessage(response.data.message)
                props.setSuccessNotifDescription('Cet eleve viens d\'etre modifie(e), vous pouvez le consulter  au catalogue avec success ! ')
                props.setShowSuccessNotif(true)
            }
        }
        })
    ...
    return (
        <div
            ...
            className="authentication-modal pt-20 pl-800"
            onClick={props.onClose}
        >
            <div className="relative items-center justify-center mx-auto  px-1 top-modal-animation" onClick={(event) => event.stopPropagation()}>
                {/* <!-- Contact Form --> */}
                <div className="bg-white bg-white rounded-sm border border-stroke shadow-default dark:border-strokedark dark:bg-boxdark">
                    ...
                    <form action="#" encType="multipart/form-data" className="modal-form">
                        <div>
                            ...
                            <div className="form-actions bg-green-600">
                                ...
                                <button onClick={props.onClose} className="flex w-1/4 ml-2 justify-center rounded bg-secondary p-3 font-medium text-white">
                                    <span className="mt-1">
                                ...
                                {props.mode !== MODAL_MODE.view && props.mode === MODAL_MODE.create ? 
                                    <button 
                                        ...
                                        disabled={isLoading}
                                        className="flex w-1/4 ml-2 justify-center rounded bg-success p-3 font-medium text-white">
