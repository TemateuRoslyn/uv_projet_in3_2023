...
import './DisplayConseilDisciplines.css';
import { AgGridIndicator } from '../../../components/AgGridIndicator';
import { ConseilDisciplinesApi } from '../../../generated';
import { TOKEN_LOCAL_STORAGE_KEY } from '../../../constants/LOCAL_STORAGE';
import { DeleteItemModal } from '../../../components/DeleteItemModal';

import { CONSEILDISCIPLINE_COLUMNS_DEFS } from '../../../configs/ag-grid-column-def/conseilDiscipline';
import CreateOrUpdateConseilDisciplineModal from './CreateOrUpdateConseilDisciplineModal';

interface DisplayConseilDisciplinesProps {
  conseilDisciplines: ConseilDiscipline[];
  isLoading: boolean;
...
const DisplayConseilDisciplines: React.FC<DisplayConseilDisciplinesProps> = (
  props
) => {
  const state = useSelector((state: ReduxProps) => state);
  const [conseilDisciplines, setConseilDisciplines] = useState<
    ConseilDiscipline[]
  >(props.conseilDisciplines);
  const [conseilDiscipline, setConseilDiscipline] = useState<ConseilDiscipline>(
    props.conseilDisciplines[0]
  );
  ...
  const onGridReady = useCallback(() => {
    ...
    conseilDisciplinesApi
      .conseilDisciplinesIndex('Bearer ' + token)
      .then((response) => {
  ...
  const handleNewItem = () => {
    ...
    setModalTitle('Créer un nouveau conseil de Discipline');
  };
  ...
  const handleExportExcel = () => {
    if (gridRef.current && gridRef.current.api) {
      ...
      if (rowData && rowData.length > 0) {
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, 'ConseilDisciplines');
        XLSX.writeFile(workbook, 'conseilDisciplines_data.xlsx');
      } else {
  ...
  const handleUpdateItem = () => {
    if (gridRef.current && gridRef.current.api) {
      ...
      if (rowData && rowData.length === 1) {
        setConseilDiscipline(rowData[0]);
        setShowCreateOrUpdateModal(true);
        setModalMode(MODAL_MODE.update);
        setModalTitle('Modifier une conseilDiscipline');
  ...
  const handleViewItem = () => {
    if (gridRef.current && gridRef.current.api) {
      ...
      if (rowData && rowData.length === 1) {
        ...
        setModalTitle("Detail d'une conseilDiscipline");
      } else if (rowData && rowData.length > 1) {
        alert(
  ...
  const proccessDeleteItem = () => {
    ...
    conseilDisciplinesApi
      .deleteConseilDiscipline('Bearer ' + token, conseilDiscipline.id)
      .then((response) => {
        if (response && response.data) {
          if (response.data.success === true) {
            ...
            props.setDangerNotifDescription(
              'Cette conseilDiscipline a ete supprimee avec success'
            );
  ...
  return (
    <div>
      ...
      {showCreateOrUpdateModal && (
        <CreateOrUpdateConseilDisciplineModal
          mode={modalMode}
          title={modalTitle}
          onClose={() => setShowCreateOrUpdateModal(false)}
          refresh={onGridReady}
          item={modalMode !== MODAL_MODE.create ? conseilDiscipline : null}
      ...
      <div className="relative col-span-12 rounded-sm border border-stroke bg-white py-6 shadow-default dark:border-strokedark dark:bg-boxdark xl:col-span-4">
        <div style={{ position: 'relative' }}>
          ...
          <div className="ag-theme-alpine" style={{ height: 500 }}>
            <AgGridReact
              ref={gridRef}
              rowData={conseilDisciplines}
              animateRows={true}
              columnDefs={CONSEILDISCIPLINE_COLUMNS_DEFS}
...
export default connect(mapStateToProps)(DisplayConseilDisciplines);
