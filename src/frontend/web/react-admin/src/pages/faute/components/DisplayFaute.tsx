...
const DisplayFaute: React.FC<DisplayFauteProps> = (props) => {
    ...
    const onGridReady = useCallback(() => {
        ...
        fautesApi.indexFautes('Bearer ' + token)
            .then((response) => {  
    ...
    const handleNewItem = () => {
        ...
        setModalTitle("Créer un nouveau faute");
    };
    ...
    const processDeleteItem = () => {
        const token: string = localStorage.getItem(TOKEN_LOCAL_STORAGE_KEY)!;
        const coursApi = new FautesApi({...state.environment, accessToken: token});

        setShowIndicator(true);

        coursApi.deleteMistake('Bearer ' + token,fauteItem.id)
            .then((response) => {  
                if (response && response.data) {                    
                    if (response.data.success === true) { 
                        ...
                        props.setDangerNotifDescription('Cette faute a été supprimé avec succès');
