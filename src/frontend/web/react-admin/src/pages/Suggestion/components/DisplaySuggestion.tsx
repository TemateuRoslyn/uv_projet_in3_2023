...
import { SUGGESTION_COLUMNS_DEFS } from '../../../configs/ag-grid-column-def/suggestion';
import { Suggestion } from '../../../generated/models';
import { ReduxProps } from '../../../redux/configureStore';
import { Link } from 'react-router-dom';
import CreateOrUpdateSuggestionModal from './CreateOrUpdateSuggestionModal';
...
const DisplaySuggestion: React.FC<DisplaySuggestionProps> = (
  props
) => {
  ...
  const [suggestionItem, setSuggestionItem] =
    useState<Suggestion>(props?.suggestion[0]);
  ...
  const handleNewItem = () => {
    ...
    setModalTitle('Créer une nouvelle suggestion');
  };
  ...
  const handleExportExcel = () => {
    if (gridRef.current && gridRef.current.api) {
      ...
      if (rowData && rowData.length > 0) {
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, 'Suggestion');
        XLSX.writeFile(workbook, 'suggestion_data.xlsx');
      } else {
  ...
  const handleUpdateItem = () => {
    if (gridRef.current && gridRef.current.api) {
      ...
      if (rowData && rowData.length === 1) {
        setSuggestionItem(rowData[0]);
        setShowCreateOrUpdateModal(true);
        setModalMode(MODAL_MODE.update);
        setModalTitle('Modifier un suggestion');
  ...
  const processDeleteItem = () => {
    ...
    suggestionApi
      .deleteSuggestion('Bearer ' + token, suggestionItem.id)
      .then((response) => {
        if (response && response.data) {
          if (response.data.success === true) {
            ...
            props.setDangerNotifDescription(
              'Cette suggestion a été supprimé avec succès'
            );
  ...
  return (
    <div>
      ...
      {showDeleteModal && (
        <DeleteItemModal
          itemName={suggestionItem.description}
        ...
        <div style={{ position: 'relative' }}>
          ...
          <div className="ag-theme-alpine" style={{ height: 500 }}>
            <AgGridReact
              ...
              rowData={suggestion}
              animateRows={true}
              columnDefs={SUGGESTION_COLUMNS_DEFS}
...
export default connect(mapStateToProps)(DisplaySuggestion);
