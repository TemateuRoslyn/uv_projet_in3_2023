...
const DisplayConvocations: React.FC<DisplayConvocationsProps> = (props) => {
  ...
  const [convocation, setConvocation] = useState<Convocation>(
    props.convocations[0]
  );
  ...
  const proccessDeleteItem = () => {
    ...
    convocationApi
      .deleteConvocation('Bearer ' + token, convocation.id)
      .then((response) => {
        if (response && response.data) {
          if (response.data.success === true) {
            ...
            props.setDangerNotifDescription(
              'Cette convocation a ete supprimee avec success'
            );
  ...
  return (
    <div>
      ...
      {showCreateOrUpdateModal && (
        <CreateOrUpdateConvocationModal
          mode={modalMode}
          title={modalTitle}
