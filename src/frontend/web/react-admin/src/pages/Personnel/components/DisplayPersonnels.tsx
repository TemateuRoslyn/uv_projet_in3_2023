...
const DisplayPersonnels: React.FC<DisplayPersonnelsProps> = (props) => {

    const state = useSelector((state: ReduxProps) => state);
    const [personnels, setPersonnels] = useState<Personnel[]>(props.personnels);
    const [personnel, setPersonnel] = useState<Personnel>(props.personnels[0]);
    ...
    const onGridReady = useCallback(() => {
        ...
        const personnelsApi = new PersonnelApi({...state.environment, accessToken: token});

        setShowIndicator(true)
        
        personnelsApi.personelIndex('Bearer ' + token)
        .then((response) => {  
        if(response && response.data){                    
            if(response.data.success === true){ 
                setPersonnels(response.data.content) 
            }
        }
        })
        .catch((error) => {
    ...
    const handleExportExcel = () => {
        if (gridRef.current && gridRef.current.api) {      
            ...
            if (rowData && rowData.length > 0) {
                ...
                XLSX.writeFile(workbook, 'personnels_data.xlsx');
            } else {
    ...
    const handleUpdateItem = () => {
        if (gridRef.current && gridRef.current.api) {      
            ...
            if (rowData && rowData.length === 1) {
                ...
                setModalTitle("Modifier une personnel")
    ...
    const proccessDeleteItem = () => {
        ...
        personnelsApi.deletePersonnel( 'Bearer ' + token, personnel.id)
        .then((response) => {  
        if(response && response.data){                    
            if(response.data.success === true){ 
                ...
                props.setDangerNotifDescription('Cette personnel a ete supprimee avec success')
  ...
  return (
      <div>
        ...
        {showDeleteModal && <DeleteItemModal 
                                itemName={personnel.firstName + '' + personnel.lastName} 
            ...
            <div style={{ position: 'relative' }}>
                ...
                <div className="ag-theme-alpine" style={{ height: 500 }}>
                    <AgGridReact 
                        ...
                        rowData={personnels} 
                        animateRows={true} 
                        columnDefs={PERSONNEL_COLUMNS_DEFS} 
...
function mapStateToProps(state: ReduxProps): ReduxProps {
  ...
  export default connect(mapStateToProps)(DisplayPersonnels)
