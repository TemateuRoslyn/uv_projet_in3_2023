...
import { Personnel, PersonnelCreateBody } from '../../../generated/models';
import { TOKEN_LOCAL_STORAGE_KEY } from '../../../constants/LOCAL_STORAGE';
import { CoursApi, PersonnelApi } from '../../../generated';
...
const CreateOrUpdatePersonnelModal: React.FC<ModalProps> = (props) => {
  ...
  const [formValues, setFormValues] = useState({
    ...
    fonction: props.item ? props.item.fonction : '',
  ...
  const handleCreate = (_event: React.FormEvent) => {
    ...
    const token: string = localStorage.getItem(TOKEN_LOCAL_STORAGE_KEY)!;
    const personnelsApi = new PersonnelApi({
      ...state.environment,
      accessToken: token,
    });
    ...
    const apiParams: PersonnelCreateBody = {
        ...
        if (response && response.data) {
          if (response.data.success === true) {
            ...
            props.setSuccessNotifDescription(
              "Une nouvelle personnel viens d'etre rajoutee au catalogue avec success ! "
            );
      ...
      .then((response) => {
        if (response && response.data) {
          if (response.data.success === true) {
            ...
            props.setSuccessNotifMessage(response.data.message);
            props.setSuccessNotifDescription(
              "Cet personnel viens d'etre modifie(e), vous pouvez le consulter  au catalogue avec success ! "
            );
            props.setShowSuccessNotif(true);
          }
        }
                ...
                <div className="mb-4.5 flex flex-col gap-6 xl:flex-row">
                  ...
                  <div className="w-full xl:w-1/2">
                    ...
                    <input
                      ...
                      required
                      type="text"
                      placeholder="Le numero du personnel"
                      className="w-full rounded border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary"
                    />
                  </div>
                </div>
...
export default CreateOrUpdatePersonnelModal;
