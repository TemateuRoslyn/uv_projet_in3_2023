...
import CreateOrUpdateClasseModal from './CreateOrUpdateClasseModal';
import { ClASSE_COLUMNS_DEFS } from '../../../configs/ag-grid-column-def/classe';
...
const DisplayClasses: React.FC<DisplayClassesProps> = (props) => {

    const state = useSelector((state: ReduxProps) => state);
    const [classes, setClasses] = useState<Classe[]>(props.classes);
    const [classe, setClasse] = useState<Classe>(props.classes[0]);
    ...
    const onGridReady = useCallback(() => {
        ...
        classesApi.classesIndex('Bearer ' + token)
        .then((response) => {  
    ...
    const handleNewItem = () => {
        ...
        setModalTitle("Créer un nouvelle classe")
    }
    ...
    const handleUpdateItem = () => {
        if (gridRef.current && gridRef.current.api) {      
            ...
            if (rowData && rowData.length === 1) {
                ...
                setModalTitle("Modifier une classe")
    ...
    const handleViewItem = () => {
        if (gridRef.current && gridRef.current.api) {      
            ...
            if (rowData && rowData.length === 1) {
                ...
                setModalTitle("Detail d'une classe")
            } else if (rowData && rowData.length > 1) {
    ...
    const proccessDeleteItem = () => {
        ...
        classesApi.classeDelete(classe.id, 'Bearer ' + token)
        .then((response) => {  
        if(response && response.data){                    
            if(response.data.success === true){ 
                ...
                props.setDangerNotifDescription('Cette classe a ete supprimee avec success')
  ...
  return (
      <div>
        ...
        {showCreateOrUpdateModal && <CreateOrUpdateClasseModal 
                                        mode={modalMode} 
                                        title={modalTitle} 
                                        onClose={() => setShowCreateOrUpdateModal(false)} 
                                        refresh={onGridReady}
                                        item={modalMode !== MODAL_MODE.create ? classe : null } 
            ...
            <div style={{ position: 'relative' }}>
                ...
                <div className="ag-theme-alpine" style={{ height: 500 }}>
                    <AgGridReact 
                        ...
                        rowData={classes} 
                        animateRows={true} 
                        columnDefs={ClASSE_COLUMNS_DEFS} 
