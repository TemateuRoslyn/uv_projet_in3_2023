...
const Classes = () => {
  ...
  useEffect(() => {  
    ...
    const classesApi = new ClassesApi({...state.environment, accessToken: apiParams});

    setIsLoading(true)
    
    classesApi.classesIndex('Bearer ' + apiParams)
    .then((response) => {  
      if(response && response.data){        
        if(response.data.success === true){ setClasses(response.data.content) }
      }
    })
    .catch((error) => {
