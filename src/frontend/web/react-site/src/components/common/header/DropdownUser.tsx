...
const DropdownUser:React.FC<DropdownUserInterface> = () => {
    ...
    <div className="relative">
      ...
      >
        <span className="hidden text-right lg:block">
          ...
          <span className="block text-xs">{authUser?.roles?.description}</span>
        </span>

        <span className="h-12 w-12 rounded-full">
          <img src={authUser?.photo} alt="User" />
        </span>
      ...
      <div
        ...
        className={` right-0 mt-1 text-black  flex w-62.5 flex-col rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark ${dropdownOpen === true ? 'block' : 'hidden'
          }`}
      >
        <ul className="flex flex-col gap-1border-b border-stroke px-6 py-7.5 dark:border-strokedark">
          <li>
            <Link
              to="/profile"
              className="flex items-center gap-3.5  text-black  text-sm font-medium duration-300 ease-in-out hover:text-primary lg:text-base"
            >
              <svg
